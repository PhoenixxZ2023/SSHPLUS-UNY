...
def c4g():
    if request.method == 'POST':
        ...
        try:
            return 'FUNCIONAL NOS APPS > UNY TUNNEL5G / DTUNNEL / CONNECTA4G'
        except Exception as e:
            return jsonify({'error': str(e)})


@app.route('/gl/check/', methods=['POST', 'GET'])
def gl(username):
...
def anymod():
    if request.method == 'POST':
        ...
        try:
            return 'FUNCIONAL NO APP > ANY VPN MOD'
        except Exception as e:
            return jsonify({'error': str(e)})
